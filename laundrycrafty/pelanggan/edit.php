...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $sql = "UPDATE pelanggan SET nama='$nama', alamat='$alamat', no_hp='$no_hp', email='$email' WHERE id_pelanggan=$id";
    
    if ($conn->query($sql)) {
        flashMessage('success', 'Data pelanggan berhasil diupdate!');
...
<head>
    ...
    <style>
        ...
        .form-card { background: white; border-radius: 15px; padding: 40px; box-shadow: 0 4px 15px rgba(0, 0, 0, 0.05); max-width: 800px; margin: 0 auto; animation: fadeInUp 0.6s ease-out; }
        @keyframes fadeInUp { from { opacity: 0; transform: translateY(30px); } to { opacity: 1; transform: translateY(0); } }
        .form-header { text-align: center; margin-bottom: 35px; padding-bottom: 20px; border-bottom: 2px solid #f3f4f6; }
        .form-header i { font-size: 60px; background: linear-gradient(135deg, var(--primary), var(--secondary)); -webkit-background-clip: text; -webkit-text-fill-color: transparent; margin-bottom: 15px; }
        .form-header h4 { color: #1f2937; font-weight: 700; margin: 0; }
        .form-group { margin-bottom: 25px; }
        .form-label { color: #374151; font-weight: 600; margin-bottom: 8px; display: block; }
        .form-label .required { color: #ef4444; }
        .form-control { padding: 12px 15px; border: 2px solid #e5e7eb; border-radius: 10px; font-size: 15px; transition: all 0.3s; width: 100%; }
        .form-control:focus { outline: none; border-color: var(--primary); box-shadow: 0 0 0 4px rgba(102, 126, 234, 0.1); }
        textarea.form-control { resize: vertical; min-height: 100px; }
        .btn-group-action { display: flex; gap: 15px; margin-top: 35px; }
        .btn-submit { flex: 1; padding: 14px; background: linear-gradient(135deg, #f59e0b, #d97706); border: none; border-radius: 10px; color: white; font-weight: 600; font-size: 16px; cursor: pointer; transition: all 0.3s; display: flex; align-items: center; justify-content: center; gap: 8px; }
        .btn-submit:hover { transform: translateY(-2px); box-shadow: 0 8px 20px rgba(245, 158, 11, 0.3); }
        .btn-cancel { flex: 1; padding: 14px; background: #e5e7eb; border: none; border-radius: 10px; color: #374151; font-weight: 600; font-size: 16px; cursor: pointer; transition: all 0.3s; text-decoration: none; display: flex; align-items: center; justify-content: center; gap: 8px; }
        .btn-cancel:hover { background: #d1d5db; color: #1f2937; }
        .alert { border-radius: 10px; padding: 15px; margin-bottom: 20px; }
        .input-icon { position: relative; }
        .input-icon i { position: absolute; left: 15px; top: 50%; transform: translateY(-50%); color: #9ca3af; }
    ...
    <div class="main-content">
        <div class="top-bar">
            <div class="page-title">
                ...
                <nav class="breadcrumb"><a href="../index.php">Dashboard</a> / <a href="list.php">Pelanggan</a> / Edit</nav>
            </div>
        </div>
